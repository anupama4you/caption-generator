...
const togglePlatform = (platform: Platform) => {
  ...
  const handleRegenerate = async (caption: Caption) => {
    if (!caption.attemptId) return;
    setRegeneratingId(caption.id);
    try {
      const response = await api.post(`/captions/attempts/${caption.attemptId}/regenerate`, {
        platform: caption.platform,
        variantNumber: caption.variantNumber,
      });
      const updated: Caption = response.data.data;
      setGeneratedCaptions(prev =>
        prev.map(c => c.id === caption.id ? { ...updated, attemptId: caption.attemptId } : c)
      );
    } catch (err) {
                          ...
                          >
                            ...
                            <div className="relative">
                                  ...
                                  {captions.map((caption) => (
                                    <div key={caption.id} className="w-full flex-shrink-0 px-1">
                                      <motion.div
                                        initial={{ opacity: 0, y: 20 }}
                                        animate={{ opacity: 1, y: 0 }}
                                        className="border-2 border-gray-100 rounded-xl p-4 hover:border-indigo-200 transition-colors bg-gradient-to-br from-white to-gray-50 relative overflow-hidden"
                                      >
                                        <AnimatePresence>
                                          {regeneratingId === caption.id && (
                                            <motion.div
                                              key="regen-overlay"
                                              ...
                                              className="absolute inset-0 rounded-xl z-20 bg-gradient-to-br from-indigo-50 via-white to-purple-50 flex flex-col items-center justify-center gap-3 border-2 border-indigo-200"
                                            >
                                              <motion.div
                                                animate={{ rotate: 360 }}
                                              ...
                                              <p className="text-sm font-bold text-indigo-700">Regenerating...</p>
                                              <p className="text-xs text-indigo-400">Crafting a fresh take ✨</p>
                                            </motion.div>
                                          )}
                                        </AnimatePresence>
                                        <div className="flex justify-between items-center mb-3">
                                          <div className="flex items-center gap-2">
                                            ...
                                            {user && caption.attemptId && (
                                              <button
                                                onClick={() => handleRegenerate(caption)}
                                                disabled={!!regeneratingId}
                                                title="Regenerate this variant"
                                                className="w-6 h-6 rounded-full bg-indigo-100 hover:bg-indigo-200 text-indigo-500 hover:text-indigo-700 flex items-center justify-center transition-all hover:scale-110 disabled:opacity-40 disabled:cursor-not-allowed"
                                              >
                              ...
                              <div className="hidden md:grid md:grid-cols-3 gap-4">
                                {captions.map((caption) => (
                                  <motion.div key={caption.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="border-2 border-gray-100 rounded-xl p-4 hover:border-indigo-200 transition-colors bg-gradient-to-br from-white to-gray-50 flex flex-col relative overflow-hidden">
                                    <AnimatePresence>
                                      {regeneratingId === caption.id && (
                                        <motion.div
                                          key="regen-overlay"
                      ...
                      <div className="relative">
                        ...
                        <div className="hidden md:grid md:grid-cols-3 gap-4">
                          ...
                          <motion.div
                            ...
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                            className="border-2 border-gray-100 rounded-xl p-4 hover:border-indigo-200 transition-colors bg-gradient-to-br from-white to-gray-50 flex flex-col relative overflow-hidden"
                          >
                            <AnimatePresence>
                              {regeneratingId === caption.id && (
                                <motion.div
